...
const PRESETS: Record<string, { name: string; getPerms: (modules: ModuleInfo[]) => Record<string, any> }> = {
  custom: {
    name: 'Custom',
    getPerms: (modules) => {
      const perms: Record<string, any> = {};
      modules.forEach(m => perms[m.id] = { canCreate: false, canRead: false, canUpdate: false, canDelete: false });
      return perms;
    }
  },
  hr_admin: {
    ...
    getPerms: (modules) => {
      ...
      modules.forEach(m => {
        if (m.id === 'staff' || m.id === 'roles') {
          perms[m.id] = { canCreate: true, canRead: true, canUpdate: true, canDelete: true };
        } else {
          perms[m.id] = { canCreate: false, canRead: false, canUpdate: false, canDelete: false };
        }
      });
      return perms;
    }
    ...
    getPerms: (modules) => {
      ...
      modules.forEach(m => {
        if (m.id === 'profile' || m.id === 'roles') {
          perms[m.id] = { canCreate: false, canRead: true, canUpdate: false, canDelete: false };
        } else {
          perms[m.id] = { canCreate: true, canRead: true, canUpdate: true, canDelete: false };
        }
      });
      return perms;
    }
...
function RolesPageContent() {
  ...
  const handleSaveRole = async () => {
    ...
    try {
      ...
      setRoles(prev => [...prev, newRole].sort((a, b) => a.roleName.localeCompare(b.roleName)));
    ...
    <div className="page-fade-in">
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ backgroundColor: 'var(--bg-primary)', padding: '2rem', borderRadius: '12px', width: '700px', maxWidth: '90vw', maxHeight: '90vh', overflowY: 'auto', border: '1px solid var(--border-color)', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h2 style={{ margin: 0, fontSize: '1.25rem' }}>Create New Role</h2>
            ...
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 600, fontSize: '0.875rem' }}>Role Name</label>
              <input 
                type="text" 
                value={newRoleName} 
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 600, fontSize: '0.875rem' }}>Permission Preset</label>
              <select 
                value={preset} 
            ...
            <h3 style={{ marginTop: '1.5rem', marginBottom: '1rem', fontSize: '1rem' }}>Module Access Configuration</h3>
            <div style={{ border: '1px solid var(--border-color)', borderRadius: '8px', overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                <thead style={{ backgroundColor: 'rgba(0,0,0,0.02)' }}>
                  <tr style={{ borderBottom: '1px solid var(--border-color)', textAlign: 'left' }}>
                    <th style={{ padding: '0.75rem 1rem', fontWeight: 600 }}>Module</th>
                    <th style={{ padding: '0.75rem', textAlign: 'center', fontWeight: 600 }}>Create</th>
                    <th style={{ padding: '0.75rem', textAlign: 'center', fontWeight: 600 }}>Read</th>
                    <th style={{ padding: '0.75rem', textAlign: 'center', fontWeight: 600 }}>Update</th>
                ...
                <tbody>
                  {modules.map(m => (
                    <tr key={m.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                      <td style={{ padding: '0.75rem 1rem', textTransform: 'capitalize' }}>{m.label}</td>
                      {(['canCreate', 'canRead', 'canUpdate', 'canDelete'] as const).map(action => (
                        <td key={action} style={{ padding: '0.75rem', textAlign: 'center' }}>
                          <input 
                            ...
                            onChange={(e) => handlePermissionChange(m.id, action, e.target.checked)}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '2rem' }}>
              <button 
                ...
                style={{ padding: '0.6rem 1.2rem', border: '1px solid var(--border-color)', borderRadius: '8px', background: 'transparent', cursor: 'pointer', fontWeight: 500 }}
              >
                Cancel
              </button>
              <button 
